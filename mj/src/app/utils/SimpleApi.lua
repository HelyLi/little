...
W = function(node) if not node then return 1 end  return SIZE(node).width end
W2 = function(node) if not node then return 1 end  return W(node)/2 end
H = function(node) if not node then return 1 end  return SIZE(node).height end
H2 = function(node) if not node then return 1 end  return H(node)/2 end
...
BOUNDING = function ( node )
    if not node then return nil end
    local box = node:getBoundingBox()
    return cc.size(box.width, box.height)
end
BW = function(node) if not node then return nil end  return BOUNDING(node).width; end
BW2 = function(node) if not node then return nil end  return BW(node)/2;end
BH = function(node) if not node then return nil end  return BOUNDING(node).height; end
BH2 = function(node) if not node then return nil end  return BH(node)/2;end

---获取坐标及设置坐标
X = function(node) if not node then return 0 end  return node:getPositionX() end
Y = function(node) if not node then return 0 end  return node:getPositionY() end
